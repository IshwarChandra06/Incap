...
public class DeviceSync {
		...
		try {
			...
			for (int i = NumberConstants.ZERO; i < jsonArray.size(); i++) {
				...
				if (null == camera) {
					...
					List<String> watchlistIdList = setWatchlistDetails(cameraObj, jsonWatchList);
					
				cameraObj.setWatchlist(watchlistIdList);
				...
				if(cameraObj.getName().contains("Entry"))
					cameraObj.setAccessType("Check In");
				else if(cameraObj.getName().contains("Exit"))
					cameraObj.setAccessType("Check Out");
	...
	private List<String> setWatchlistDetails(Device cameraObj, JSONArray jsonWatchList) {
		List<String> watchlistIdList= new ArrayList<String>();
		if(null != jsonWatchList || !(jsonWatchList.isEmpty())) {
			for (int k = NumberConstants.ZERO; k < jsonWatchList.size(); k++) {
				...
				watchlistIdList.add(watchlistId);

			}
			
		}
		return watchlistIdList;
	}
